process.title = "AKAMESHOP | V13 OwO Hunt Bot"
...
import { commandHandler, randomInt, sleep, solveCaptcha, timeHandler } from "./lib/extension.mjs"
...
process.on("SIGINT", function () {
    console.log("\n")
    console.log("\x1b[92mGönderilen toplam komut: \x1b[0m" + global.totalcmd)
    console.log("\x1b[92mGönderilen toplam mesaj: \x1b[0m" + global.totaltext)
    console.log("\x1b[92mToplam aktif süre: \x1b[0m" + timeHandler(global.startTime, Date.now()))
    console.log("\x1b[36mSELFBOT SONLANDIRILDI!\x1b[0m")
    process.exit(1)
});
...
(async () => {
    ...
    client
    .on("ready", async () => {
        log(client.user.username + " olarak giriş yapıldı.", "i")
    ...
    .on("messageCreate", async (message) => {
        if(message.author.id == global.owoID) {
            if(((message.content.includes(message.client.user.username) || 
                    ...
                    if(message.attachments) {
                        try {
                            ...
                            if(!attempt || attempt.match(/\d/)) {
                                log("Captcha çözme sistemi geri dönüyor: " + attempt, "i");
                                throw new Error();
                            }
                            ...
                            log("Captcha çözme denemesi başarısız oldu.", "e")
                            console.log(error);
                            return await notify(message)
                        }
                    }
                    else log("Captcha görseli bulunamadı.", "i")
                }
                else log("TEKRAR BAŞLATMAK İÇİN CAPTCHANIN ÇÖZÜLMESİ BEKLENİYOR...", "i")
            }

            else if(message.content.match(/verified that you are.{1,3}human!/igm) && message.channel.type == 'DM') {
                log(`CAPTCHA ÇÖZÜLDÜ${global.config.autoResume ? ", SELFBOT TEKRAR BAŞLATILIYOR..." : ""}`, "i");
                ...
                log("HESAP YASAKLANMIŞ, SELFBOT DURDURULUYOR...", "e")
            ...
            if(message.channel.type == "DM" && global.captchaDetected && message.channel.recipient.id === global.config.userNotify) {
                if(message.content.match(/^[a-zA-Z]{3,6}$/)) {
                    ...
                    try {
                        ...
                        msgr.reply("Bir hata oluştu, lütfen hesabı kendiniz kontrol edinç")
                    }
                } else {
                    return msgr.reply("Geçersiz sözdizimi, bu mesaj OwO Bot'a gönderilmeyecek.")
                }
            } 
        } 
    }).on("messageCreate", async message => {
